...
class Inventory {
	...
	public function deleteCategory($categoryId) {
		// Start a transaction
		mysqli_begin_transaction($this->dbConnect);
	
		try {
			// Delete products associated with the category
			$deleteProductsQuery = "DELETE FROM " . $this->productTable . " WHERE categoryid = ?";
			$stmt = mysqli_prepare($this->dbConnect, $deleteProductsQuery);
			mysqli_stmt_bind_param($stmt, 'i', $categoryId);
			mysqli_stmt_execute($stmt);
			mysqli_stmt_close($stmt);
	
			// Delete brands associated with the category
			$deleteBrandsQuery = "DELETE FROM " . $this->brandTable . " WHERE categoryid = ?";
			$stmt = mysqli_prepare($this->dbConnect, $deleteBrandsQuery);
			mysqli_stmt_bind_param($stmt, 'i', $categoryId);
			mysqli_stmt_execute($stmt);
			mysqli_stmt_close($stmt);
	
			// Delete the category itself
			$deleteCategoryQuery = "DELETE FROM " . $this->categoryTable . " WHERE categoryid = ?";
			$stmt = mysqli_prepare($this->dbConnect, $deleteCategoryQuery);
			mysqli_stmt_bind_param($stmt, 'i', $categoryId);
			mysqli_stmt_execute($stmt);
			mysqli_stmt_close($stmt);
	
			// Commit the transaction
			mysqli_commit($this->dbConnect);
	
			echo "Category and all associated brands and products deleted successfully.";
		} catch (Exception $e) {
			// Roll back the transaction in case of error
			mysqli_rollback($this->dbConnect);
